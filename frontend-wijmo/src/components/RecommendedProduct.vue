            ...
            <v-row cols="12">
                <v-col cols="6">
                    <div style="display:flex;">
                        <div class="label-title">ProductType</div>
                        <String label="입력하세요." v-model="value.productType" :editMode="editMode"/>
                    </div>
                </v-col>
                <v-col cols="6">
                    <div style="display:flex;">
                        <div class="label-title">Size</div>
                        <String label="입력하세요." v-model="value.size" :editMode="editMode"/>
                    </div>
                </v-col>
                <v-col cols="6">
                    <div style="display:flex;">
                        <div class="label-title">Gender</div>
                        <String label="입력하세요." v-model="value.gender" :editMode="editMode"/>
                    </div>
                </v-col>
                <v-col cols="6">
                    <div style="display:flex;">
                        <div class="label-title">Preferences</div>
                        <String label="입력하세요." v-model="value.preferences" :editMode="editMode"/>
                    </div>
                </v-col>
            </v-row>
        </div>
...
import RecommendedProductBase from '../components/RecommendedProductBase.vue'
...
export default {
    ...
    created(){
        ...
        websocketUrl.pathname = "/rsocket/recommendedProducts";
        websocketUrl.hash = "";
        ...
        client.connect().subscribe({
            ...
            const endpoint = "recommendedProducts."+ me.value.productType +".get"
    ...
    methods: {
        ...
        async save(){
            try {
                ...
                if(!this.offline) {
                    if(this.isNew) {
                        temp = await axios.post(axios.fixUrl('/recommendedProducts'), this.value)
                    } else {
