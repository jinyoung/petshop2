<template>
    <div style="max-height:80vh;">
        ...
        <RecommendationByPetIdQuery @search="search"></RecommendationByPetIdQuery>
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['productType','size','age','gender','preferences','photo','petProfileId',]" />
            ...
            <wj-flex-grid-column binding="petProfileId." header="petProfile" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">RecommendedProduct 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <div>
                        <RecommendedProduct :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="recommendedProductModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import RecommendationByPetIdQuery from '../components/RecommendationByPetIdQuery.vue';
...
export default {
    name : 'recommendedProduct-grid',
    mixins:[RecommendedProduct],
    components:{
        RecommendationByPetIdQuery,
        RecommendedProduct,
    },
    data: () => ({
        flex: null,
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.recommendedProduct.href;
