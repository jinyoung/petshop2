<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            추천 제품 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            추천 제품
        </v-card-title >        
        ...
        <v-card-text>
            <String v-if="editMode" label="ProductType" v-model="value.productType" :editMode="editMode" :inputUI="''"/>
